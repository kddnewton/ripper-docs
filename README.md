...
If you'd like to see the syntax tree that Ripper generates for Ruby code, you can try Ripper in irb. The sexp_raw() method shows the parser nodes/events for the code, while lex() shows the scanner events. You can also use the sexp() method to group events in a more readable way:
